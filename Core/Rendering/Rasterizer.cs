...
public static class Rasterizer
{
    ...
    //OLD CODE will remove after new one is perfect!
    /*public static void RasterizeMeshSolidZ(
        Mesh m,
        in SN.Matrix4x4 world, in SN.Matrix4x4 view, in SN.Matrix4x4 proj,
        uint[] color, float[] zbuf, int W, int H,
                    ...
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        if (mat?.Textures != null && mat.Textures.Count > 0)
                        {
                            foreach (var slot in mat.Textures)
                            {
                                ...
                                var tex = rawObj as Texture2D ?? TextureBridge.EnsureEngineTexture2D(rawObj);
    ...
    //OLD CODE 
    /* private static float GetF(object o, string name, float def)
     {
     ...
     private static bool GetB(object o, string name, bool def)
     {
         var p = o.GetType().GetProperty(name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
         if (p == null) return def;
         var v = p.GetValue(o);
     ...
     private static void ApplyUVXform(object slot, ref float u, ref float v)
     {
