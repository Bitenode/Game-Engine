...
namespace Game_Engine.Core.Rendering.ShaderGraph
{
    ...
    internal sealed class ShaderGraphCompiler
    {
        ...
        public (string vertex, string fragment) Compile()
        {
            ...
            fragSb.AppendLine("in vec3 vWorldNormal;");
            ...
            fragSb.AppendLine("uniform sampler2D uTexture0;   // Albedo");
            fragSb.AppendLine("uniform sampler2D uTexture1;   // Normal map");
            fragSb.AppendLine("uniform sampler2D uTexture2;   // Specular map");
            fragSb.AppendLine("uniform sampler2D uTexture3;");
            fragSb.AppendLine("uniform int   uHasNormalMap;");
            fragSb.AppendLine("uniform float uNormalStrength;");
            fragSb.AppendLine("uniform int   uHasSpecularMap;");
            fragSb.AppendLine("uniform float uRoughness;");
            fragSb.AppendLine("uniform float uMetallic;");
            fragSb.AppendLine("uniform float uAmbient;");
            fragSb.AppendLine();
            fragSb.AppendLine("// ── Light uniforms (uLightDir = FROM light, negate in shader) ──");
            ...
            fragSb.AppendLine();
            fragSb.AppendLine("#define PI 3.14159265359");
            fragSb.AppendLine();
            fragSb.AppendLine("// Cotangent-frame TBN for normal mapping");
            fragSb.AppendLine("mat3 CotangentFrame(vec3 N, vec3 p, vec2 uv) {");
            fragSb.AppendLine("    vec3 dp1 = dFdx(p); vec3 dp2 = dFdy(p);");
            fragSb.AppendLine("    vec2 duv1 = dFdx(uv); vec2 duv2 = dFdy(uv);");
            fragSb.AppendLine("    vec3 dp2perp = cross(dp2, N); vec3 dp1perp = cross(N, dp1);");
            fragSb.AppendLine("    vec3 T = dp2perp * duv1.x + dp1perp * duv2.x;");
            fragSb.AppendLine("    vec3 B = dp2perp * duv1.y + dp1perp * duv2.y;");
            fragSb.AppendLine("    float mx = max(dot(T,T), dot(B,B));");
            fragSb.AppendLine("    if (mx < 1e-6) return mat3(vec3(1,0,0), vec3(0,1,0), N);");
            fragSb.AppendLine("    float inv = inversesqrt(mx);");
            fragSb.AppendLine("    return mat3(T*inv, B*inv, N);");
            fragSb.AppendLine("}");
            fragSb.AppendLine();
            fragSb.AppendLine("// GGX Distribution");
            fragSb.AppendLine("float DistGGX(vec3 N, vec3 H, float r) {");
            fragSb.AppendLine("    float a = r*r; float a2 = a*a;");
            fragSb.AppendLine("    float d = max(dot(N,H),0.0); d = d*d*(a2-1.0)+1.0;");
            fragSb.AppendLine("    return a2 / max(PI*d*d, 0.0001);");
            fragSb.AppendLine("}");
            fragSb.AppendLine("float GeoGGX(float NdV, float r) {");
            fragSb.AppendLine("    float k = ((r+1.0)*(r+1.0))/8.0;");
            fragSb.AppendLine("    return NdV / (NdV*(1.0-k)+k);");
            fragSb.AppendLine("}");
            fragSb.AppendLine("float GeoSmith(vec3 N, vec3 V, vec3 L, float r) {");
            fragSb.AppendLine("    return GeoGGX(max(dot(N,V),0.0),r) * GeoGGX(max(dot(N,L),0.0),r);");
            fragSb.AppendLine("}");
            fragSb.AppendLine("vec3 FresnelSchlick(float cosT, vec3 F0) {");
            fragSb.AppendLine("    return F0 + (1.0-F0)*pow(clamp(1.0-cosT,0.0,1.0), 5.0);");
            fragSb.AppendLine("}");
            fragSb.AppendLine();
            ...
            string normal = GetInputExpression(_graph.Output.Inputs[1], "normalize(vWorldNormal)");
            string metallic = GetInputExpression(_graph.Output.Inputs[2], "uMetallic");
            string roughness = GetInputExpression(_graph.Output.Inputs[3], "uRoughness");
            ...
            fragSb.AppendLine();
            fragSb.AppendLine("    // ── Surface setup ──");
            fragSb.AppendLine($"    vec3 albedoVal = {albedo};");
            fragSb.AppendLine($"    vec3 N = normalize(vWorldNormal);");
            fragSb.AppendLine($"    vec3 geoN = N;");
            fragSb.AppendLine();
            fragSb.AppendLine("    // Normal mapping from material texture");
            fragSb.AppendLine("    if (uHasNormalMap == 1) {");
            fragSb.AppendLine("        vec3 mapN = texture(uTexture1, vTexCoord).rgb * 2.0 - 1.0;");
            fragSb.AppendLine("        mapN.xy *= uNormalStrength;");
            fragSb.AppendLine("        mapN = normalize(mapN);");
            fragSb.AppendLine("        mat3 TBN = CotangentFrame(N, vWorldPos, vTexCoord);");
            fragSb.AppendLine("        N = normalize(TBN * mapN);");
            fragSb.AppendLine("        if (dot(N, geoN) < 0.1) N = normalize(mix(N, geoN, 0.5));");
            fragSb.AppendLine("    }");
            fragSb.AppendLine();
            fragSb.AppendLine($"    float metallicVal = {metallic};");
            fragSb.AppendLine($"    float roughnessVal = clamp({roughness}, 0.04, 1.0);");
            fragSb.AppendLine($"    vec3 emissionVal = {emission};");
            fragSb.AppendLine($"    float opacityVal = {opacity};");
            fragSb.AppendLine($"    float aoVal = {ao};");
            fragSb.AppendLine();
            fragSb.AppendLine("    // Specular map modulation");
            fragSb.AppendLine("    float specMask = 1.0;");
            fragSb.AppendLine("    if (uHasSpecularMap == 1) {");
            fragSb.AppendLine("        specMask = texture(uTexture2, vTexCoord).r;");
            fragSb.AppendLine("        roughnessVal = mix(roughnessVal, 1.0 - specMask, 0.3);");
            fragSb.AppendLine("    }");
            fragSb.AppendLine();
            fragSb.AppendLine("    // ── Cook-Torrance PBR ──");
            fragSb.AppendLine("    vec3 L = normalize(-uLightDir);");
            fragSb.AppendLine("    vec3 V = normalize(uCameraPos - vWorldPos);");
            fragSb.AppendLine("    vec3 H = normalize(V + L);");
            fragSb.AppendLine("    float NdotL = max(dot(N, L), 0.0);");
            fragSb.AppendLine();
            fragSb.AppendLine("    vec3 F0 = mix(vec3(0.04), albedoVal, metallicVal);");
            fragSb.AppendLine("    float NDF = DistGGX(N, H, roughnessVal);");
            fragSb.AppendLine("    float G   = GeoSmith(N, V, L, roughnessVal);");
            fragSb.AppendLine("    vec3  F   = FresnelSchlick(max(dot(H, V), 0.0), F0);");
            fragSb.AppendLine("    vec3 spec = (NDF * G * F) / max(4.0 * max(dot(N,V),0.0) * NdotL + 0.0001, 0.0001);");
            fragSb.AppendLine("    vec3 kD = (vec3(1.0) - F) * (1.0 - metallicVal);");
            fragSb.AppendLine("    float intensity = max(uLightIntensity, 0.0);");
            fragSb.AppendLine("    vec3 lightCol = uLightColor;");
            fragSb.AppendLine("    if (dot(lightCol, lightCol) < 0.001) lightCol = vec3(1.0);");
            fragSb.AppendLine("    vec3 radiance = lightCol * intensity;");
            fragSb.AppendLine("    vec3 Lo = (kD * albedoVal / PI + spec) * radiance * NdotL;");
            fragSb.AppendLine();
            fragSb.AppendLine("    // Ambient (fallback to 0.15 if not set)");
            fragSb.AppendLine("    float amb = uAmbient;");
            fragSb.AppendLine("    if (amb < 0.01) amb = 0.15;");
            fragSb.AppendLine("    vec3 ambient = vec3(amb) * albedoVal * aoVal;");
            fragSb.AppendLine("    vec3 finalColor = ambient + Lo + emissionVal;");
            fragSb.AppendLine();
            fragSb.AppendLine("    // Reinhard tone mapping");
            fragSb.AppendLine("    finalColor = finalColor / (finalColor + vec3(1.0));");
