...
public static class VegetationProfileLibrary
{
    ...
    static void EnsureDefault(Dictionary<string, VegetationProfile> map)
    {
        ...
        EnsureStub(map, "Savanna", 0.35f, 0.12f, 0.55f);
        EnsureStub(map, "Wetlands", 0.7f, 0.2f, 0.4f);
        EnsureStub(map, "Boreal", 0.55f, 0.55f, 0.5f);
        EnsureStub(map, "Ice", 0.02f, 0f, 0.7f);
        EnsureStub(map, "Alpine", 0.2f, 0.08f, 0.55f);
        EnsureStub(map, "Desert", 0.08f, 0.02f, 0.65f);
        EnsureStub(map, "Volcanic", 0.05f, 0.01f, 0.7f);
        EnsureStub(map, "Forest", 0.8f, 0.6f, 0.4f);
        EnsureStub(map, "Grassland", 0.5f, 0.15f, 0.45f);
        EnsureStub(map, "Tundra", 0.15f, 0.02f, 0.55f);
        EnsureStub(map, "Ocean", 0f, 0f, 0.5f);
        EnsureStub(map, "Universal", 0.4f, 0.2f, 0.45f);
    }

    static void EnsureStub(Dictionary<string, VegetationProfile> map, string id,
        float veg, float tree, float patch)
    {
        if (map.ContainsKey(id)) return;
        map[id] = new VegetationProfile
        {
            Id = id,
            VegetationDensity = veg,
            TreeDensity = tree,
            VegetationPatchiness = patch,
            GrassModelPath = "Assets/Standard Assets/Planet Vegetation/Grass/Meadow_Grass_01_Var4.FBX",
        };
        EnsureLegacyCompatibility(map[id]);
    }
