...
| **`SaveManager`** | Slot JSON saves **`ISaveable`** behaviors plus **`SceneName`**. **`Load(slot)`** calls **`SceneManager.LoadScene`** when the saved scene differs, then applies `ISaveable` data on **`SceneLoaded`**. Use **`LoadIntoCurrentScene`** to skip the scene change. **`Terrain`** / **`PlanetTerrain`** are **not** `ISaveable` by default—use terrain/planet asset files for world shape, or implement **`ISaveable`** on a wrapper if you need slot data to reference asset paths. |
